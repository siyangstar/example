...
public class FindFragment extends Fragment implements OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        v.findViewById(R.id.btnSet_find).setOnClickListener(this);
        mBtnRemind = v.findViewById(R.id.btnRemind_find);
        mIvHeadImg = v.findViewById(R.id.ivHeadImg_find);
        mTvName = v.findViewById(R.id.tvName_find);
        mIvNewSet = v.findViewById(R.id.ivNewSet_find);
        showNewMessage();// 根据是否收到消息进行提醒显示
        ...
        if (Globals.g_userInfo != null) {
            ...
            mTvName.setText(Globals.g_userInfo.nickname);
        }
    ...
    public void onClick(View v) {
        ...
        } else if (v.getId() == R.id.btnSet_find) {// 设置
...
//			getActivity().startActivity(intent);
        } else if (v.getId() == R.id.llMyLottery_find) {
            // 正式跳转
...
//			Intent intent = new Intent(getActivity(), WaitingActivity.class);
            ...
            getActivity().startActivity(intent);
        } else if (v.getId() == R.id.llMessage_find) {
            mBtnRemind.setVisibility(View.GONE);
            Intent intent = new Intent(getActivity(), MessageCenterActivity.class);
            startActivity(intent);
        } else if (v.getId() == R.id.llCollection_find) {
    ...
    public void onResume() {
        ...
        if (mTvName.getText().toString().length() == 0 && Globals.g_userInfo != null) {
            refreshHeader(Globals.g_userInfo.headUrl);
            mTvName.setText(Globals.g_userInfo.nickname);
        }
        if (Globals.g_userInfo == null) {
    ...
    public void showNewMessage() {
        if (SharedPreferencesInfo.getTagBoolean(getActivity(), SharedPreferencesInfo.NEW_MESSAGE, false)) {
            ...
            if (num != 0) {
                ...
                mBtnRemind.setText(String.valueOf(num));
            } else {
                mBtnRemind.setVisibility(View.INVISIBLE);
                SharedPreferencesInfo.setTagBoolean(getActivity(), SharedPreferencesInfo.NEW_MESSAGE, false);
            }
        } else {
            mBtnRemind.setVisibility(View.INVISIBLE);
        }
    ...
    public void showNewSetting() {
        if (SharedPreferencesInfo.getTagBoolean(getActivity(), SharedPreferencesInfo.NEW_SETTING, false)
                && (SharedPreferencesInfo.getTagBoolean(getActivity(), SharedPreferencesInfo.NEW_SUGGEST, false)
                || SharedPreferencesInfo.getTagBoolean(getActivity(), SharedPreferencesInfo.NEW_VERSION, false))) {
            mIvNewSet.setVisibility(View.VISIBLE);
        } else {
            mIvNewSet.setVisibility(View.GONE);
